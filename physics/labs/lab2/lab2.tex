...
Show that under uniformly accelerated motion, the following statements hold true:
\begin{itemize}
...
% \end{align*}
% }
% \equationboxx{Kinematic Quantity Definitions}{
% \begin{align*}
% x &=  \text{position [cm]} \\ 
% t &=  \text{time [ms]} \\
% v &= \text{speed [cm/ms]} \\ 
% \Delta x &= \text{displacement (the \textbf{difference} between two positions)}\\
% \Delta t &= \text{time interval (the \textbf{difference} between two times)}\\
% v &= \Delta x / \Delta t = \text{(speed is the change in position divided by change in time)}
% \end{align*}
% }
